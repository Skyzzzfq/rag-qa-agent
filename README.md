...
基于 RAG + Agent 技术的智能文档问答系统，支持上传 PDF/Markdown 文档，自动构建知识库，并通过自然语言进行问答。
...
- **ReAct Agent**：基于推理-行动模式的智能体，自主选择工具完成复杂问答任务
- **反思纠错**：LLM 自动检查回答的事实性、完整性和一致性，不通过则重试（最多 2 次）
- **中文优化**：文本切分使用中文优先分隔符，BM25 使用字符级分词，适配中文场景
- **增量索引**：支持文档增量添加，无需每次重建全部索引
- **会话隔离**：基于 session_id 的对话记忆管理，支持多用户并发
